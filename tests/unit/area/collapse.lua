...
test.sequence = coroutine.create(function()
    ...
    assert(collapse_state.cutout.right == area_bounds.left + 100, "cutout not matching content despite not cut off")
    assert(collapse_state.cutout.bottom == area_bounds.top + 100, "cutout not matching content despite not cut off")
    ...
    for fac = 0, 99 do
        ...
        assert(collapse_state.cutout.right ~= area_bounds.left + 100, "cutout still matching content in width despite factor not 1")
        assert(collapse_state.cutout.bottom == area_bounds.top + 100, "cutout wrong")
        -- width should be smaller than rectangle
        assert(area_bounds.right - area_bounds.left < 100, "area width should be smaller than contents")
        -- in fact it should correspond to content width * width factor
        assert(area_bounds.right == area_bounds.left + 100 * collapse_state.width_factor, "area width should match content width * width factor")
    end
    ...
    for fac = 0, 99 do
        ...
        assert(collapse_state.cutout.left == area_bounds.left, "cutout wrong")
        assert(collapse_state.cutout.top == area_bounds.top, "cutout wrong")
        assert(collapse_state.cutout.right == area_bounds.left + 100, "cutout wrong")
        assert(collapse_state.cutout.bottom ~= area_bounds.top + 100, "cutout still matching content in height despite factor not 1")
        -- height should be smaller than content
        assert(area_bounds.bottom - area_bounds.top < 100, "area height should be smaller than contents")
        -- in fact it should correspond to content height * height factor
        assert(area_bounds.bottom == area_bounds.top + 100 * collapse_state.height_factor, "area width should match content height * height factor")
    end
end)
