...
return function(state, min, max, step, decimals, format)
    ...
    -- set base shape
    local full_width = math.max(const.numeric_input_min_width, cursor.width)
    cursor.place(full_width, const.numeric_input_height)

    local center_width = cursor.width - const.numeric_input_lr_button_width * 2
    ...
    if hovering or dragging == mb.left then
        ...
        cursor.width = const.numeric_input_lr_button_width
        cursor.change_anchor(0.5)
        ...
        draw_by_cursor.icon("chevron-left", const.numeric_input_text_size)
        -- #endregion
        ...
        draw_by_cursor.icon("chevron-right", const.numeric_input_text_size)
        -- #endregion
    end
    ...
    if is_editing then
        typing.draw_text_entry(const.numeric_input_text_size, "input")
    else
        draw_by_cursor.label(string.format(format or "%f", state.value), const.numeric_input_text_size, "left", false)
    end
